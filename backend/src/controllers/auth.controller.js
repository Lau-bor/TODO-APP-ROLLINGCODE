...
export const register = async (req, res) => {
  try {
    ...
    const userFound = await User.findOne({ email }); // Pregunto a mongo si existe un usuario con el email que envía el cliente

    if (userFound)
      return res.status(400).json({ message: "User already exist" }); // si existe retornamos mensaje de error

    const passwordHash = await bcrypt.hash(password, 12); //hasheo la passs con un salt de 12
    ...
    // 1 enviar el email para la verificacion
...
export const login = async (req, res) => {
  try {
    ...
    const isMatch = await bcrypt.compare(password, userFound.password); // comparo la contraseña enviada contra la del usuario encontrado en mongo

    if (!isMatch)
      return res.status(400).json({ message: "Invalid credentials" }); // si la contraseñas no son iguales doy mensaje de error
